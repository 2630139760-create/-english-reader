...
const BACKUP_META_KEY = "english-context-reader-backup-meta-v1";
const IMPORT_SNAPSHOT_KEY = "english-context-reader-import-snapshot-v1";
...
function copyVocabulary(saved) {
  ...
  Object.entries(saved).forEach(([key, raw]) => {
    ...
    result[finalKey] = { ...previous, ...raw, type, text, word: raw.word || text, reviewStatus: ["unfamiliar","learning","mastered"].includes(raw.reviewStatus) ? raw.reviewStatus : (previous?.reviewStatus || "unfamiliar"), reviewCount: Number.isInteger(raw.reviewCount) && raw.reviewCount >= 0 ? raw.reviewCount : (previous?.reviewCount || 0), lastReviewedAt: typeof raw.lastReviewedAt === "string" ? raw.lastReviewedAt : (previous?.lastReviewedAt || null), lastReviewResult: ["unfamiliar","learning","mastered"].includes(raw.lastReviewResult) ? raw.lastReviewResult : (previous?.lastReviewResult || null), createdAt: previous?.createdAt || raw.createdAt || now(), sources: [...(previous?.sources || []), ...sources].filter((item, index, all) => all.findIndex(x => x.articleId === item.articleId && x.source === item.source) === index) };
...
function makeVocabularyItem(key, entry, articleId = null) {
  ...
  button.textContent=`${entry.type === "phrase" ? "短语" : "单词"} · ${reviewStatusLabel(entry.reviewStatus)} · ${entry.text} · ${entry.meaning || "暂无中文释义"} · ${sourceForArticle(entry, articleId)}`;
...
function reviewStatusLabel(value) { return ({unfamiliar:"陌生",learning:"学习中",mastered:"已掌握"})[value] || "陌生"; }
function renderVocabulary() { const entries=Object.entries(state.vocabulary).filter(([,e])=>(vocabularyFilter==="all"||e.type===vocabularyFilter)&&(vocabularyStatusFilter==="all"||e.reviewStatus===vocabularyStatusFilter)).sort((a,b)=>a[1].text.localeCompare(b[1].text,"en")); const total=Object.keys(state.vocabulary).length; vocabularyCount.textContent=String(total); vocabularySummary.textContent=`显示 ${entries.length} / 共 ${total} 个`; vocabularyEmpty.hidden=entries.length>0; vocabularyItems.replaceChildren(); entries.forEach(([key,e])=>vocabularyItems.append(makeVocabularyItem(key,e))); }
function renderArticleVocabulary() { const unit=state.articles.find(x=>x.id===articleVocabularyId); if(!unit)return; const all=Object.entries(state.vocabulary).filter(([,e])=>isEntryInArticle(e,unit.id)); const shown=all.filter(([,e])=>(articleVocabularyFilter==="all"||e.type===articleVocabularyFilter)&&(articleVocabularyStatusFilter==="all"||e.reviewStatus===articleVocabularyStatusFilter)); articleVocabularyTitle.textContent=`《${unit.articleTitle}》的词汇`; articleVocabularySummary.textContent=`显示 ${shown.length} / 共 ${all.length} 个`; articleVocabularyEmpty.hidden=shown.length>0; articleVocabularyItems.replaceChildren(); shown.sort((a,b)=>a[1].text.localeCompare(b[1].text,"en")).forEach(([key,e])=>articleVocabularyItems.append(makeVocabularyItem(key,e,unit.id))); }
...
function writeBackupMeta(patch){const value={...readBackupMeta(),...patch};localStorage.setItem(BACKUP_META_KEY,JSON.stringify(value));return value;}
function makeBackup(){const exportedAt=now();return{application:"英语语境阅读器",schemaVersion:BACKUP_SCHEMA_VERSION,exportedAt,statistics:dataCounts(),data:{library:JSON.parse(JSON.stringify(state)),settings:{currentArticleId:state.currentId}}};}
function backupJSON(){return JSON.stringify(makeBackup(),null,2);}
function formatBackupName(date=new Date()){const pad=n=>String(n).padStart(2,"0");return`english-reader-backup-${date.getFullYear()}-${pad(date.getMonth()+1)}-${pad(date.getDate())}-${pad(date.getHours())}${pad(date.getMinutes())}.json`;}
function setDataStatus(message,type="success"){dataStatus.textContent=message;dataStatus.className=`import-status is-${type}`;}
function renderDataPanel(){const counts=dataCounts();dataStats.innerHTML=`<div class="data-stat"><strong>${counts.articles}</strong><span>文章</span></div><div class="data-stat"><strong>${counts.words}</strong><span>单词</span></div><div class="data-stat"><strong>${counts.phrases}</strong><span>短语</span></div><div class="data-stat"><strong>${counts.vocabulary}</strong><span>词汇总数</span></div>`;const meta=readBackupMeta();lastExportTime.textContent=meta.lastExportAt?new Date(meta.lastExportAt).toLocaleString("zh-CN"):"暂无";lastImportTime.textContent=meta.lastImportAt?new Date(meta.lastImportAt).toLocaleString("zh-CN"):"暂无";undoImportButton.hidden=!localStorage.getItem(IMPORT_SNAPSHOT_KEY);}
function openDataPanel(event){dataTrigger=event?.currentTarget||document.activeElement;renderDataPanel();dataPanel.hidden=false;dataBackdrop.hidden=false;document.body.style.overflow="hidden";dataPanelClose.focus();}
function closeDataPanel(){dataPanel.hidden=true;dataBackdrop.hidden=true;document.body.style.overflow="";if(dataTrigger&&document.contains(dataTrigger))dataTrigger.focus();dataTrigger=null;}
function validateLibrary(raw){if(!isPlainObject(raw)||!Array.isArray(raw.articles)||!isPlainObject(raw.vocabulary))throw new Error("备份数据不完整：缺少文章库或词汇表。");const articles=raw.articles.map((article,index)=>{if(!isPlainObject(article)||!isPlainObject(article.original)||typeof article.original.english!=="string")throw new Error(`备份中的第 ${index+1} 篇文章不完整。`);if(article.scenes!==undefined&&(!Array.isArray(article.scenes)||article.scenes.length>5))throw new Error(`备份中的第 ${index+1} 篇文章场景格式错误或超过 5 个。`);return normalizeUnit(article)});const vocabulary=copyVocabulary(raw.vocabulary);return{version:5,currentId:typeof raw.currentId==="string"?raw.currentId:null,articles,vocabulary};}
function parseBackup(text){let raw;try{raw=JSON.parse(text)}catch{throw new Error("JSON 格式无效，请检查文件是否完整。")}let oldVersion=false,library,createdAt,schemaVersion;if(raw?.application==="英语语境阅读器"&&Number.isInteger(raw.schemaVersion)){if(raw.schemaVersion<1||raw.schemaVersion>BACKUP_SCHEMA_VERSION)throw new Error(`不支持的数据版本：${raw.schemaVersion}。`);if(typeof raw.exportedAt!=="string"||!isPlainObject(raw.statistics)||!isPlainObject(raw.data)||!isPlainObject(raw.data.library))throw new Error("备份缺少导出时间、数据统计或 data.library 必要字段。");library=raw.data.library;createdAt=raw.exportedAt;schemaVersion=raw.schemaVersion}else if(isPlainObject(raw)&&Array.isArray(raw.articles)&&isPlainObject(raw.vocabulary)){library=raw;oldVersion=true;createdAt=raw.exportedAt;schemaVersion=raw.version||"旧版"}else{throw new Error("这不是有效的英语语境阅读器备份。")}const normalized=validateLibrary(library);const duplicateArticles=normalized.articles.filter(a=>state.articles.some(b=>b.id===a.id||(normalizeText(b.original.title)===normalizeText(a.original.title)&&normalizeText(b.original.english)===normalizeText(a.original.english)))).length;const duplicateVocabulary=Object.keys(normalized.vocabulary).filter(key=>state.vocabulary[key]).length;return{library:normalized,createdAt:typeof createdAt==="string"?createdAt:null,schemaVersion,oldVersion,duplicateArticles,duplicateVocabulary};}
function previewBackup(text){const parsed=parseBackup(text);pendingBackup=parsed;const c=dataCounts(parsed.library);backupPreviewDetails.textContent=`创建时间：${parsed.createdAt?new Date(parsed.createdAt).toLocaleString("zh-CN"):"旧备份未记录"}；数据版本：${parsed.schemaVersion}；文章 ${c.articles} 篇；单词 ${c.words} 个；短语 ${c.phrases} 个；词汇共 ${c.vocabulary} 个；旧版本数据：${parsed.oldVersion?"是（将自动迁移）":"否"}；可能重复：文章 ${parsed.duplicateArticles} 篇、词汇 ${parsed.duplicateVocabulary} 个；安全导入：是。同名但内容不同的文章将保留双方并标注“导入”。`;backupPreview.hidden=false;setDataStatus("备份验证通过，请选择导入方式。");return parsed;}
function uniqueImportedTitle(title,articles){let result=`${title}（导入）`,n=2;while(articles.some(x=>x.articleTitle===result))result=`${title}（导入 ${n++}）`;return result;}
function mergeLibrary(imported){const result=validateLibrary(JSON.parse(JSON.stringify(state)));const idMap={};let addedArticles=0,skippedArticles=0,addedWords=0,addedPhrases=0,mergedRelations=0,renamedArticles=0;for(const incoming of imported.articles){const exact=result.articles.find(x=>normalizeText(x.original.title)===normalizeText(incoming.original.title)&&normalizeText(x.original.english)===normalizeText(incoming.original.english));if(exact){idMap[incoming.id]=exact.id;skippedArticles++;continue}let article=JSON.parse(JSON.stringify(incoming));if(result.articles.some(x=>x.id===article.id))article.id=uid();if(result.articles.some(x=>normalizeText(x.articleTitle)===normalizeText(article.articleTitle))){article.articleTitle=uniqueImportedTitle(article.articleTitle,result.articles);renamedArticles++}idMap[incoming.id]=article.id;result.articles.push(article);addedArticles++}for(const [key,incoming] of Object.entries(imported.vocabulary)){const mappedSources=(incoming.sources||[]).map(x=>({...x,articleId:idMap[x.articleId]||x.articleId}));const existing=result.vocabulary[key];if(!existing){result.vocabulary[key]={...incoming,sources:mappedSources};if(incoming.type==="phrase")addedPhrases++;else addedWords++;continue}const sources=[...(existing.sources||[])];for(const source of mappedSources){if(!sources.some(x=>x.articleId===source.articleId&&x.source===source.source)){sources.push(source);mergedRelations++}}const newest=(incoming.lastReviewedAt||"")>(existing.lastReviewedAt||"")?incoming:existing;result.vocabulary[key]={...incoming,...existing,meaning:existing.meaning||incoming.meaning||"",phonetic:existing.phonetic||incoming.phonetic||"",example:existing.example||incoming.example||"",reviewStatus:newest.reviewStatus||"unfamiliar",reviewCount:Math.max(existing.reviewCount||0,incoming.reviewCount||0),lastReviewedAt:newest.lastReviewedAt||null,lastReviewResult:newest.lastReviewResult||null,sources}}if(!result.currentId)result.currentId=result.articles[0]?.id||null;return{result,summary:{addedArticles,skippedArticles,addedWords,addedPhrases,mergedRelations,renamedArticles}};}
function commitImport(next,summary,oldVersion){const before=localStorage.getItem(STORAGE_KEY);const priorSnapshot=localStorage.getItem(IMPORT_SNAPSHOT_KEY);try{localStorage.setItem(IMPORT_SNAPSHOT_KEY,JSON.stringify({createdAt:now(),library:JSON.parse(before||JSON.stringify(state))}));localStorage.setItem(STORAGE_KEY,JSON.stringify(next));writeBackupMeta({lastImportAt:now()});state=next}catch(error){if(before===null)localStorage.removeItem(STORAGE_KEY);else localStorage.setItem(STORAGE_KEY,before);if(priorSnapshot===null)localStorage.removeItem(IMPORT_SNAPSHOT_KEY);else localStorage.setItem(IMPORT_SNAPSHOT_KEY,priorSnapshot);throw new Error("浏览器存储空间不足，导入未生效，原数据已恢复。")}fillEditor(currentUnit());renderLibrary();renderVocabulary();renderDataPanel();const migrated=oldVersion?"；已迁移旧版本数据":"";setDataStatus(`导入完成：新增 ${summary.addedArticles} 篇文章，跳过 ${summary.skippedArticles} 篇重复文章，新增 ${summary.addedWords} 个单词、${summary.addedPhrases} 个短语，合并 ${summary.mergedRelations} 条词汇关联${summary.renamedArticles?`，${summary.renamedArticles} 篇同名异文已另存`:""}${migrated}。`);pendingBackup=null;backupPreview.hidden=true;}


function reviewSnapshot() { return { queue:[...reviewQueue], index:reviewIndex }; }
function selectedReviewValue(name, fallback) { return document.querySelector(`input[name="${name}"]:checked`)?.value || fallback; }
function matchingReviewKeys(options = currentReviewOptions()) {
  let entries = Object.entries(state.vocabulary).filter(([, entry]) =>
    (!options.articleId || isEntryInArticle(entry, options.articleId)) &&
    (options.type === "all" || entry.type === options.type) &&
    (options.status === "all" || entry.reviewStatus === options.status));
  entries.sort((a, b) => (a[1].createdAt || "").localeCompare(b[1].createdAt || ""));
  return entries.map(([key]) => key);
}
function currentReviewOptions() { return { articleId: reviewScopeArticleId, type:selectedReviewValue("reviewType","all"), status:selectedReviewValue("reviewStatus","all"), limit:selectedReviewValue("reviewLimit","10"), order:selectedReviewValue("reviewOrder","random") }; }
...
function updateReviewMatchCount() { const count=matchingReviewKeys().length; reviewMatchCount.textContent=count?`符合当前条件：${count} 个词汇`:`没有符合当前条件的词汇，请调整筛选条件。`;beginReviewButton.disabled=!count; }
function openReview(articleId=null, trigger=null) {
  reviewScopeArticleId=articleId;reviewTrigger=trigger||document.activeElement;reviewSetup.hidden=false;reviewSession.hidden=true;reviewComplete.hidden=true;
  const unit=articleId&&state.articles.find(x=>x.id===articleId);reviewScopeLabel.textContent=unit?`复习范围：当前文章《${unit.articleTitle}》`:`复习范围：全部文章的全局词汇`;
  reviewPanel.hidden=false;reviewBackdrop.hidden=false;document.body.style.overflow="hidden";updateReviewMatchCount();beginReviewButton.focus();
}
function closeReview() { window.speechSynthesis?.cancel?.();reviewPanel.hidden=true;reviewBackdrop.hidden=true;document.body.style.overflow="";renderVocabulary();if(articleVocabularyId)renderArticleVocabulary();if(reviewTrigger&&document.contains(reviewTrigger))reviewTrigger.focus();reviewTrigger=null; }
function startReview(keys=null) {
  reviewOptions=currentReviewOptions();let candidates=keys||matchingReviewKeys(reviewOptions);if(reviewOptions.order==="random")candidates=shuffle(candidates);
  const limit=reviewOptions.limit==="all"?candidates.length:Number(reviewOptions.limit);reviewQueue=candidates.slice(0,limit);if(!reviewQueue.length){updateReviewMatchCount();return;}
  reviewIndex=0;reviewHistory=[];reviewResults={};reviewUndo.disabled=true;reviewSetup.hidden=true;reviewComplete.hidden=true;reviewSession.hidden=false;renderReviewCard();
}
function reviewSources(entry) {
  const sources=[...(entry.sources||[])].sort((a,b)=>Number(b.articleId===reviewScopeArticleId)-Number(a.articleId===reviewScopeArticleId));
  if(!sources.length)return "来源文章：未归属文章\n来源原句：暂无";
  return sources.map(source=>{const title=state.articles.find(x=>x.id===source.articleId)?.articleTitle||"已删除的文章";return `来源文章：${title}\n来源原句：${source.source||"暂无"}`}).join("\n\n");
}
function renderReviewCard() {
  const key=reviewQueue[reviewIndex],entry=state.vocabulary[key];if(!entry){reviewQueue.splice(reviewIndex,1);if(!reviewQueue.length)return finishReview();reviewIndex=Math.min(reviewIndex,reviewQueue.length-1);return renderReviewCard();}
  reviewRevealed=false;reviewProgress.textContent=`${reviewIndex+1} / ${reviewQueue.length}`;reviewTypeBadge.textContent=entry.type==="phrase"?"短语":"单词";reviewWord.textContent=entry.text;
  reviewAnswer.textContent="";reviewAnswer.hidden=true;reviewAnswerHint.hidden=false;showReviewAnswer.hidden=false;reviewStatusActions.hidden=true;reviewPrevious.disabled=reviewIndex===0;reviewNext.disabled=reviewIndex===reviewQueue.length-1;
}
function revealReviewAnswer() {
  if(reviewSession.hidden||reviewRevealed)return;const entry=state.vocabulary[reviewQueue[reviewIndex]];if(!entry)return;
  const fields=[`中文释义：${entry.meaning||"暂无"}`,`音标：${entry.phonetic||"暂无"}`,reviewSources(entry),`例句：${entry.example||"暂无"}`,`例句中文翻译：${entry.exampleTranslation||entry.exampleChinese||"暂无"}`];
  reviewAnswer.textContent=fields.join("\n");reviewAnswer.hidden=false;reviewAnswerHint.hidden=true;showReviewAnswer.hidden=true;reviewStatusActions.hidden=false;reviewRevealed=true;
}
function goReview(delta) { const next=reviewIndex+delta;if(next<0||next>=reviewQueue.length)return;reviewIndex=next;renderReviewCard(); }
function recordReview(result) {
  if(!reviewRevealed)return;const key=reviewQueue[reviewIndex],entry=state.vocabulary[key];if(!entry)return;
  reviewHistory.push({key,index:reviewIndex,reviewStatus:entry.reviewStatus,reviewCount:entry.reviewCount,lastReviewedAt:entry.lastReviewedAt,lastReviewResult:entry.lastReviewResult});
  entry.reviewStatus=result;entry.reviewCount=(entry.reviewCount||0)+1;entry.lastReviewedAt=now();entry.lastReviewResult=result;reviewResults[key]=result;saveState();reviewUndo.disabled=false;
  if(reviewIndex===reviewQueue.length-1)finishReview();else{reviewIndex++;renderReviewCard();}
}
function undoReview() {
  const previous=reviewHistory.pop();if(!previous)return;const entry=state.vocabulary[previous.key];if(entry){entry.reviewStatus=previous.reviewStatus;entry.reviewCount=previous.reviewCount;entry.lastReviewedAt=previous.lastReviewedAt;entry.lastReviewResult=previous.lastReviewResult;saveState();}
  delete reviewResults[previous.key];reviewIndex=previous.index;reviewSession.hidden=false;reviewComplete.hidden=true;renderReviewCard();revealReviewAnswer();reviewUndo.disabled=!reviewHistory.length;
}
function finishReview() {
  reviewSession.hidden=true;reviewComplete.hidden=false;const values=Object.values(reviewResults);const entries=reviewQueue.map(key=>state.vocabulary[key]).filter(Boolean);
  const count=status=>values.filter(x=>x===status).length;reviewSummary.textContent=`本轮复习总数：${reviewQueue.length}\n陌生：${count("unfamiliar")} · 学习中：${count("learning")} · 已掌握：${count("mastered")}\n单词：${entries.filter(x=>x.type!=="phrase").length} · 短语：${entries.filter(x=>x.type==="phrase").length}`;
}
function speakReview() { const entry=state.vocabulary[reviewQueue[reviewIndex]];if(!entry||!("speechSynthesis" in window))return;const utterance=new SpeechSynthesisUtterance(entry.text);utterance.lang="en-US";const voices=window.speechSynthesis.getVoices();utterance.voice=voices.find(v=>v.lang==="en-US")||voices.find(v=>v.lang.startsWith("en"))||null;window.speechSynthesis.cancel();window.speechSynthesis.speak(utterance); }
...
vocabularyStatusFilters.addEventListener("click",e=>{const b=e.target.closest("button[data-status]");if(!b)return;vocabularyStatusFilter=b.dataset.status;[...vocabularyStatusFilters.querySelectorAll("button")].forEach(x=>x.setAttribute("aria-pressed",String(x===b)));renderVocabulary();});
startReviewButton.addEventListener("click",e=>openReview(null,e.currentTarget));reviewArticleButton.addEventListener("click",e=>openReview(articleVocabularyId,e.currentTarget));
articleVocabularyStatusFilters.addEventListener("click",e=>{const b=e.target.closest("button[data-status]");if(!b)return;articleVocabularyStatusFilter=b.dataset.status;[...articleVocabularyStatusFilters.querySelectorAll("button")].forEach(x=>x.setAttribute("aria-pressed",String(x===b)));renderArticleVocabulary();});
reviewSetup.addEventListener("change",updateReviewMatchCount);beginReviewButton.addEventListener("click",()=>startReview());reviewCard.addEventListener("click",revealReviewAnswer);showReviewAnswer.addEventListener("click",revealReviewAnswer);reviewSpeak.addEventListener("click",speakReview);reviewStatusActions.addEventListener("click",e=>{const b=e.target.closest("button[data-review-result]");if(b)recordReview(b.dataset.reviewResult);});reviewPrevious.addEventListener("click",()=>goReview(-1));reviewNext.addEventListener("click",()=>goReview(1));reviewUndo.addEventListener("click",undoReview);[reviewExit,reviewClose,reviewReturn].forEach(x=>x.addEventListener("click",closeReview));reviewBackdrop.addEventListener("click",closeReview);reviewAgain.addEventListener("click",()=>startReview());reviewDifficult.addEventListener("click",()=>{const difficult=reviewQueue.filter(key=>["unfamiliar","learning"].includes(reviewResults[key]));if(difficult.length)startReview(difficult);else reviewSummary.textContent+="\n本轮没有标记为陌生或学习中的词汇。";});
...
dataManagementButton.addEventListener("click",openDataPanel);dataPanelClose.addEventListener("click",closeDataPanel);dataBackdrop.addEventListener("click",closeDataPanel);
exportDataButton.addEventListener("click",()=>{try{const json=backupJSON();if(typeof Blob!=="undefined"&&globalThis.URL?.createObjectURL){const blob=new Blob([json],{type:"application/json;charset=utf-8"});const link=document.createElement("a");link.href=URL.createObjectURL(blob);link.download=formatBackupName();document.body.append(link);link.click();link.remove?.();URL.revokeObjectURL(link.href)}writeBackupMeta({lastExportAt:now()});renderDataPanel();setDataStatus("全部数据已成功导出。若浏览器未下载，可使用“复制备份 JSON”。")}catch{setDataStatus("导出失败，请检查浏览器存储或使用复制功能。","error")}});
copyBackupButton.addEventListener("click",async()=>{const json=backupJSON();try{if(globalThis.navigator?.clipboard?.writeText)await globalThis.navigator.clipboard.writeText(json);else{backupText.value=json;backupText.select?.();if(!document.execCommand?.("copy"))throw new Error()}writeBackupMeta({lastExportAt:now()});renderDataPanel();setDataStatus("完整备份 JSON 已复制。") }catch{backupText.value=json;setDataStatus("浏览器未允许自动复制，完整 JSON 已放入文本框，请手动复制。","error")}});
previewBackupButton.addEventListener("click",()=>{try{previewBackup(backupText.value)}catch(error){pendingBackup=null;backupPreview.hidden=true;setDataStatus(error.message,"error")}});
backupFile.addEventListener("change",()=>{const file=backupFile.files?.[0];if(!file)return;if(!file.name.toLowerCase().endsWith(".json")){setDataStatus("请选择 .json 备份文件。","error");return}const reader=new FileReader();reader.onload=()=>{backupText.value=String(reader.result||"");try{previewBackup(backupText.value)}catch(error){pendingBackup=null;backupPreview.hidden=true;setDataStatus(error.message,"error")}};reader.onerror=()=>setDataStatus("无法读取所选文件，请重新选择。","error");reader.readAsText(file,"UTF-8")});
confirmImportButton.addEventListener("click",()=>{if(!pendingBackup){setDataStatus("请先验证并预览备份。","error");return}const selected=document.querySelector('input[name="importMode"]:checked');const mode=selected?.value||"merge";if(mode==="replace"&&(!window.confirm("覆盖导入会使用备份替换当前应用数据。是否继续？")||!window.confirm("请再次确认：覆盖前会保存可撤销快照，确定替换吗？")))return;try{if(mode==="merge"){const merged=mergeLibrary(pendingBackup.library);commitImport(merged.result,merged.summary,pendingBackup.oldVersion)}else{const next=validateLibrary(pendingBackup.library);const c=dataCounts(next);commitImport(next,{addedArticles:c.articles,skippedArticles:0,addedWords:c.words,addedPhrases:c.phrases,mergedRelations:0,renamedArticles:0},pendingBackup.oldVersion)}}catch(error){setDataStatus(error.message||"导入失败，现有数据没有改变。","error")}});
undoImportButton.addEventListener("click",()=>{if(!window.confirm("撤销会恢复最近一次导入前的数据，是否继续？")||!window.confirm("请再次确认恢复导入前快照。"))return;try{const snapshot=JSON.parse(localStorage.getItem(IMPORT_SNAPSHOT_KEY));const restored=validateLibrary(snapshot?.library);localStorage.setItem(STORAGE_KEY,JSON.stringify(restored));state=restored;localStorage.removeItem(IMPORT_SNAPSHOT_KEY);fillEditor(currentUnit());renderLibrary();renderVocabulary();renderDataPanel();setDataStatus("已撤销最近一次导入并恢复原数据。")}catch{setDataStatus("撤销失败，当前数据没有改变。","error")}});
function openArticleVocabulary(id){articleVocabularyId=id;articleVocabularyFilter="all";articleVocabularyStatusFilter="all";libraryItems.hidden=true;libraryEmpty.hidden=true;newArticleButton.hidden=true;saveArticleButton.hidden=true;articleVocabulary.hidden=false;libraryTitle.textContent="本篇词汇";[...articleVocabularyFilters.querySelectorAll("button")].forEach(button=>button.setAttribute("aria-pressed",String(button.dataset.filter==="all")));[...articleVocabularyStatusFilters.querySelectorAll("button")].forEach(button=>button.setAttribute("aria-pressed",String(button.dataset.status==="all")));renderArticleVocabulary();articleVocabularyBack.focus();}
