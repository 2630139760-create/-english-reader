...
let phraseMode = false, phraseStart = null, pendingPhrase = null, vocabularyFilter = "all";
...
function renderLibrary(){libraryItems.replaceChildren();libraryEmpty.hidden=state.articles.length>0;state.articles.slice().sort((a,b)=>b.updatedAt.localeCompare(a.updatedAt)).forEach(unit=>{const card=document.createElement("article");card.className="library-card";const related=Object.values(state.vocabulary).filter(e=>!e.articleId||e.articleId===unit.id).length;const info=document.createElement("div"),heading=document.createElement("h3"),chinese=document.createElement("p"),meta=document.createElement("p");heading.textContent=unit.original.title||unit.articleTitle;chinese.className="library-chinese";chinese.textContent=unit.original.chineseTitle||"暂无中文标题";meta.className="library-meta";meta.textContent=`${unit.scenes.length} 个场景 · ${related} 个词汇 · ${new Date(unit.updatedAt).toLocaleString("zh-CN")}`;info.append(heading);info.append(chinese);info.append(meta);card.append(info);[["open","打开"],["continue","继续阅读"],["rename","重命名"],["delete","删除"]].forEach(([action,label])=>{const b=document.createElement("button");b.type="button";b.dataset.action=action;b.dataset.id=unit.id;b.textContent=label;b.setAttribute("aria-label",`${label}文章 ${unit.articleTitle}`);if(action==="delete")b.className="danger-text";card.append(b);});libraryItems.append(card);});}
function openLibrary(){renderLibrary();libraryPanel.hidden=false;libraryBackdrop.hidden=false;document.body.style.overflow="hidden";libraryClose.focus();}
function closeLibrary(){libraryPanel.hidden=true;libraryBackdrop.hidden=true;document.body.style.overflow="";libraryButton.focus();}
...
libraryButton.addEventListener("click",openLibrary);libraryClose.addEventListener("click",closeLibrary);libraryBackdrop.addEventListener("click",closeLibrary);saveArticleButton.addEventListener("click",()=>saveCurrent(true));
newArticleButton.addEventListener("click",()=>{state.currentId=uid();fillEditor(null);saveState();closeLibrary();showEditor();});
